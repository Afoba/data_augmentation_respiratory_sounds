...
def build_plan(
    ...
    exact=False (default): "top up to a total" mode. n_needed = max(0,
    target - n_real - n_prev_augmented) — target means the desired
    combined real+synthetic count for that class. This is what you want
    for correcting class imbalance (e.g. minority classes augmented up
    toward the majority class's real count).

    exact=True: "generate exactly N new" mode. n_needed = max(0, target -
    n_prev_augmented) — n_real is ignored entirely. This is what you want
    when the synthetic count itself is the quantity that matters (e.g. "500
    vertically-flipped copies of each class," independent of how many real
    cycles that class happens to have) — no more real_count + N arithmetic
    needed, and unlike the arithmetic trick, this stays correct even if
    n_real shifts later (a different train/val split, more source data,
    etc.), since it was never a function of n_real to begin with.
    Re-running is still idempotent either way: n_prev_augmented (rows
    already generated by a matching --generator-prefix) is always
    subtracted, so a second run tops up any shortfall rather than
    duplicating work.
    """
    ...
    for cls in all_classes:
        ...
        if exact:
            n_needed = max(0, cls_target - n_prev_aug)
        else:
            n_needed = max(0, cls_target - n_real - n_prev_aug)
        summary_rows.append({
            "class": cls, "n_real": n_real, "n_prev_augmented": n_prev_aug,
            "target": cls_target, "n_to_generate": n_needed, "mode": "exact" if exact else "top_up",
        })
...
def main():
    ...
    parser.add_argument("--target", type=int, default=None,
                        help="Uniform target sample count per class. Meaning depends on --exact: "
                             "'top up to this total' by default, or 'generate exactly this many new "
                             "samples, ignoring real count' if --exact is set.")
    parser.add_argument("--target-map", nargs="+", default=None,
                        help='Per-class overrides, e.g. crackle=2000 wheeze=2000')
    parser.add_argument("--exact", action="store_true",
                        help="Treat --target/--target-map as an exact new-sample count instead of a "
                             "top-up total — ignores how many real cycles exist for each class. Use "
                             "this instead of manually computing real_count + N when the number of "
                             "synthetic samples itself is what matters (e.g. reproducing a paper's "
                             "'N flipped copies per class' rather than correcting class imbalance).")
